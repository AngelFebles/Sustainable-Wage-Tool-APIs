...
def dowload_job_salary_data(county):
    ...
    try:
        ...
        driver.find_element(
            By.XPATH, '//a[@href="/pub/time.series/oe/oe.data.1.AllData"]'
        ).click()
        time.sleep(1)
    ...
    ## At this point, we only identify each job by its ID, so (for convenience) we will add the job titles to the DataFrame

    # Add a new column to the DataFrame (This will be used to store the job title)
    df.insert_column(0, pl.lit("").alias("Job_Title"))

    # Get the job titles
    titles_df = pl.read_csv(
        os.path.join(os.path.dirname(__file__), "DataFiles/oe.occupation"),
        separator="\t",
    )
    titles_df = titles_df.select(titles_df.columns[:2])

    # Ensure the 'ids' column is of type string
    df = df.with_columns(pl.col("ids").cast(pl.Int64))

    # Merge the DataFrames on the 'ids' column to get the job titles of only the jobs inside our df
    reordered_df = df.join(titles_df, on="ids", how="left")

    # The occupation names are appended at the end of the DataFrame, so we need to move them to the front
    foo = reordered_df["occupation_name"]
    reordered_df = reordered_df.drop(reordered_df.columns[-1])
    reordered_df.replace_column(0, foo)
...
def split_df(df, county):
    ...
    df_only_from_county = df.filter(
        pl.col("series_id                     ").str.contains(county_header)
    )
...
def file_to_df(path, county):
    ...
    df_only_from_county_onlyIDS = df_only_from_county_onlyIDS.unique(
        subset="ids", maintain_order=True
    )

    # print(df_only_from_county_onlyIDS)

    combined_df = pl.concat(
        [df_only_from_county_onlyIDS, reshaped_df], how="horizontal"
    )
...
def get_education_requirements():
    ...
    for row in table.find_all("tr"):
        ...
        elements_in_row = [
            element.text.strip() for element in elements_in_row if element.text.strip()
        ]
    ...
    df_education_requirements = df_education_requirements.drop(
        df_education_requirements.columns[-1]
    )

    # Remove the '-' from the SOC code
    df_education_requirements = df_education_requirements.with_columns(
        pl.col(df_education_requirements.columns[1])
        .str.replace("-", "")
        .alias(df_education_requirements.columns[1])
    )
...
def jobDataScrapeStarter(county):
    ...
    with xlsxwriter.Workbook(
        "src\sustainable_wage_tool_data\DataFiles\OutputFiles\jobData.xlsx"
    ) as workbook:
        job_data_df.write_excel(workbook=workbook, worksheet="Job_Data")
        educational_requirements_df.write_excel(
            workbook=workbook, worksheet="Education_Requirements"
        )


# county = "Racine, WI"
# jobDataScrapeStarter(county)
