...
def getAgeCohortMeans(df):
    ...
    for category in categories:
        mean = (
            df.filter(pl.col("Age Cohort") == category)
            .select(
                [
                    "Age Cohort",
                    "Thrifty Monthly",
    ...
    meansDF = df.group_by("Age Cohort").agg(
        pl.col(
            ["Thrifty Monthly", "Low Monthly", "Moderate Monthly", "Liberal Monthly"]
        ).mean()
    )
    ...
    preschooler_row = meansDF.filter(pl.col("Age Cohort") == "Preschooler").select(
        pl.col("*")
    )
    schoolager_row = meansDF.filter(pl.col("Age Cohort") == "School Age").select(
        pl.col("*")
    )
    teenager_row = meansDF.filter(pl.col("Age Cohort") == "Teenager").select(
        pl.col("*")
    )

    meansDF = pl.concat(
        [adult_row, infant_row, preschooler_row, schoolager_row, teenager_row]
    )
